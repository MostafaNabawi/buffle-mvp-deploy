...
const TaskManagerPreogressBar = (props) => {
  ...
  const handlePlay = async () => {
    if (!play) {
      ...
      setPlay(!play);
      if (seconds > 30) {
        const sp_time = `${days + parseInt(time[0])}:${
          hours + parseInt(time[1])
        }:${minutes + parseInt(time[2]) + 1}`;
  ...
  useEffect(async () => {
    setPercent(
      (100 *
        ((days + parseInt(time[0])) * 86400 +
          (hours + parseInt(time[1])) * 3600 +
          (minutes + parseInt(time[2])) * 60 +
          seconds +
          range)) /
        duration
    );
        ...
        <Col xl="11" className="pl-0">
          ...
          <ProgressBar
            label={
              <>
                <span>
                  {days + parseInt(time[0]) + day}:
                  {hours + parseInt(time[1]) + hour}:
                  {minutes + parseInt(time[2]) + min}:{seconds}
                </span>
              </>
            }
            now={percent}
